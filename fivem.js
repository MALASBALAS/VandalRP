...
client.on("ready", () => {
  var statuss = [`${client.users.size} Usuarios | ${config.prefix}help`]
  setInterval(function start() {
    log(`(${client.user.username} Bot): ${colors.red(`El bot de VandalRP esta: ONLINE.`)}`);
...
client.on("message", (message) => {
  if(message.content.startsWith("$cache")) {
    message.channel.send("Buenas Ciudadano tienes problemas con FiveM? prueba borrando cache! Aqui te dejo un video de como borrarlo! https://streamable.com/wrm5he")
  }
